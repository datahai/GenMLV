...
def ensure_schemas_exist(required_schemas, dry_run=False):
    existing_schemas = {db.name for db in spark.catalog.listDatabases()}
...
def drop_obsolete_mlvs(sql_files, mlv_metadata, dry_run=False):
    ...
    for schema in all_schemas:
        try:
            for table in spark.catalog.listTables(schema):
                existing_mlvs.add((schema, table.name))
        except Exception as e:
            print(f"⚠️ Could not list tables in schema '{schema}': {e}")
    ...
    for schema, mlv in mlvs_to_drop:
        print(f"{'Would drop' if dry_run else 'Dropping'} obsolete table: {schema}.{mlv}")
        if not dry_run:
            try:
                spark.catalog.dropTable(f"{schema}.{mlv}")
                print(f"✅ Dropped obsolete table: {schema}.{mlv}")
                mlv_metadata.pop(f"{schema}.{mlv}", None)
            except Exception as e:
                print(f"❌ Failed to drop table '{schema}.{mlv}': {e}")
...
def create_or_update_mlvs(sql_files, existing_mlvs, mlv_metadata, dry_run=False):
    ...
    for file_info in ordered_sql_files:
        ...
        if modified_datetime > last_processed_datetime:
            ...
            print(f"{'Would create/replace' if dry_run else 'Creating or replacing'} table: {schema}.{table_name}")
            if not dry_run:
                try:
                    ...
                    print(f"✅ Table '{schema}.{table_name}' created or replaced successfully.")
                    ...
                    print(f"❌ Failed to create/replace table '{schema}.{table_name}': {e}")
